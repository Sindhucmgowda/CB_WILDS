...
parser.add_argument('--group', required = True, choices = ['gender', 'ethnicity', 'insurance'])
...
run_name = "protected_group" + time.strftime("-%Y-%m-%d_%H_%M_%S")
...
train_data = data_cxr.get_dataset(envs = ['MIMIC'], split = 'train', only_frontal = False, subset_label = args.group)
valid_data = data_cxr.get_dataset(envs = ['MIMIC'], split = 'val', only_frontal = False, subset_label = args.group)
test_data = data_cxr.get_dataset(envs = ['MIMIC'], split = 'test', only_frontal = False, subset_label = args.group)
...
model_conv = models.get_model('CXR', 'Conf', 'none', args.use_pretrained, n_outputs = 2 if args.group == 'gender' else 3).to(device)
